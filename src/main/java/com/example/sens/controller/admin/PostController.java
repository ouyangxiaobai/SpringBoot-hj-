...
public class PostController extends BaseController {
    ...
    public String posts(Model model,
        ...
        postCondition.setPostStatus(status);
    ...
    public JsonResult pushPost(@ModelAttribute Post post) {
        boolean isAgency = loginUserIsAgency();
        if (isAgency) {
            if (StringUtils.isEmpty(post.getOwnerName()) || StringUtils.isEmpty(post.getOwnerPhone())) {
                return JsonResult.error("中介发布房源时请填写房东姓名和联系方式");
            }
        }

        User user = userService.get(getLoginUserId());
        if (StringUtils.isEmpty(user.getPhone()) || StringUtils.isEmpty(user.getEmail())) {
            return JsonResult.error("请先在个人资料中填写手机号和电子邮箱信息");
        }
        ...
        // 3.处理月租金展示
        if (post.getDayPrice() != null) {
            post.setMonthPrice(post.getDayPrice() * 30);
        }

        // 4.添加/更新入库
        if (post.getId() == null) {
            post.setAgencyFlag(isAgency);
            post.setUserId(getLoginUserId());
        } else {
            Post temp = postService.get(post.getId());
            if (temp == null) {
                return JsonResult.error("保存失败，房屋信息不存在");
            }
            post.setAgencyFlag(temp.isAgencyFlag());
        }
